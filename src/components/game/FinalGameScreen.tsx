...
import { Trophy, Medal, Award, Users, Clock, TrendingUp, Sparkles } from 'lucide-react';
...
export function FinalGameScreen({ 
  ...
  const calculatePlayerStats = (player: Player): PlayerStats => {
    ...
    return {
      ...
      successRate: estimatedBids > 0 ? (estimatedPerfectBids / estimatedBids) * 100 : 0,
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default: return <span className="w-8 h-8 flex items-center justify-center text-gray-500 font-bold text-lg">#{rank}</span>;
    }
  };
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600">
            {session.gameType === 'up' ? '10 Sections' : '20 Sections'} • {session.scoringSystem === 'classic' ? 'Classic' : 'Modern'} Scoring
          </p>
        </div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="text-xl font-bold">
                      {player.name}
                      {player.isAI && <span className="ml-2 text-sm opacity-75">(AI)</span>}
                    </h4>
                    {index === 0 && <p className="text-sm opacity-90">Champion!</p>}
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      {stats.rank === 1 && <Trophy className="w-5 h-5 text-yellow-500" />}
                      {stats.rank === 2 && <Medal className="w-5 h-5 text-gray-400" />}
                      {stats.rank === 3 && <Award className="w-5 h-5 text-amber-600" />}
                    </div>
                    <div className="text-right">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="text-center p-2 bg-gray-50 rounded">
                      ...
                      <div className="text-lg font-bold text-gray-900">{stats.totalScore}</div>
                    </div>
                    <div className="text-center p-2 bg-gray-50 rounded">
                    ...
                    <div className="text-center p-2 bg-green-50 rounded">
                      ...
                      <div className="text-lg font-bold text-green-600">{stats.perfectBids}</div>
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-jossing-primary">{totalSections}</div>
              <div className="text-sm text-gray-600">Sections Played</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-jossing-primary">{players.length}</div>
            ...
            <div>
              <div className="text-2xl font-bold text-jossing-primary">
                {Math.max(...Object.values(totalScores)) - Math.min(...Object.values(totalScores))}
              </div>
        ...
        <div className="text-center mt-8">
          <p className="text-gray-600 mb-4">Share these results with friends!</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              type="button"
              onClick={() => {
                const results = `🃏 Jøssing Game Results 🃏\n\n🏆 Winner: ${winner.name} (${winner.finalScore} pts)\n\n` +
                  sortedPlayers.map((p, i) => `${i + 1}. ${p.name}: ${p.finalScore} pts`).join('\n') +
            ...
            <button
              ...
              onClick={() => {
                ...
                if (navigator.share) {
                  ...
                  navigator.clipboard.writeText(`Play Jøssing with me! ${gameUrl}`).then(() => {
