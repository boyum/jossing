...
export class MediumAI extends BaseAI {
   ...
   */
  makeBid(hand: Card[], maxBid: number, trumpSuit: Suit, position: number, opponentBids: number[]): number {
   ...
   */
  playCard(hand: Card[], currentTrick: Card[], trumpSuit: Suit, leadingSuit?: Suit, gameContext?: GameContext): Card {
    ...
    return this.followStrategically(validCards, currentTrick, trumpSuit, leadingSuit, gameContext);
  }

  /**
   * More sophisticated trick estimation
   */
  private estimateAdvancedTricks(handStrength: HandStrength, trumpSuit: Suit): number {
   ...
   */
  private analyzeOpponentBids(opponentBids: number[], handSize: number): number {
   ...
   */
  private leadStrategically(validCards: Card[], trumpSuit: Suit, gameContext?: GameContext): Card {
    if (!gameContext) {
  ...
  private followStrategically(
    ...
    const tricksNeeded = gameContext ? this.calculateTricksNeeded(gameContext) : 0;
    const canWin = validCards.some(card => this.canWinTrick(card, currentTrick, trumpSuit, leadingSuit));
    
    if (tricksNeeded > 0 && canWin) {
      // Need tricks and can win - play strategically to win
      return this.winTrickEfficiently(validCards, currentTrick, trumpSuit, leadingSuit);
    } else if (tricksNeeded <= 0) {
      // Don't need tricks - try to lose or play safely
      return this.avoidWinningTrick(validCards, currentTrick, trumpSuit, leadingSuit);
    } else {
  ...
  private leadForTricks(validCards: Card[], trumpSuit: Suit): Card {
    ...
    return trumps.find(card => RANK_VALUES[card.rank] >= 10 && RANK_VALUES[card.rank] <= 12) || trumps[0];
  }
   ...
   */
  private winTrickEfficiently(validCards: Card[], currentTrick: Card[], trumpSuit: Suit, leadingSuit?: Suit): Card {
    ...
    const nonTrumpWinners = winningCards.filter(card => card.suit !== trumpSuit);
   ...
   */
  private avoidWinningTrick(validCards: Card[], currentTrick: Card[], trumpSuit: Suit, leadingSuit?: Suit): Card {
