...
export abstract class BaseAI {
	...
	protected calculateHandStrength(hand: Card[], trumpSuit: Suit): HandStrength {
		...
		const voidSuits = Object.values(suitCounts).filter(
			(count) => count === 0,
		).length;
		const shortSuits = Object.values(suitCounts).filter(
			(count) => count === 1,
		).length;
	...
	protected getHighestCard(
		...
		return trick.reduce((highest, current) => {
			return this.compareCards(current, highest, trumpSuit, leadingSuit) > 0
				? current
				: highest;
		});
	}
