...
export const useGameStore = create<GameStore>((set, get) => ({
	...
	startPlayingPhase: async (sessionId: string) => {
		...
		try {
			const result = await gameApiService.startPlayingPhase(
				sessionId,
				playerId,
			);
			if (result.success) {
	...
	addAI: async (sessionId: string, difficulty?: string) => {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Failed to add AI players";
			set({
				error: errorMessage,
				isConnected: false,
				isLoading: false,
	...
	removeAI: async (sessionId: string, playerId: string) => {
		...
		try {
			set({ isLoading: true, error: null });
			const result = await gameApiService.removeAIPlayer(
				sessionId,
				playerId,
				adminPlayerId,
			);
